...
function getMusic( file ){
	...
	waveformCreate( file, function( result ){
		console.log("waveformCreate result : ");
		console.log(result);
...
function waveformCreate( file, callback ) {
	console.log("waveformCreate");
	var reader = new FileReader();
	reader.onload = function(e) {
		console.log("waveformCreate reader");
		console.log(e.target.result);
		context.decodeAudioData( e.target.result, function( buffer ) {
			console.log("waveformCreate context.decode");
			console.log(buffer);
			var channel = buffer.getChannelData(0);
			var sections = Math.floor( audio.duration * 5 );
			var len = Math.floor( channel.length / sections );
			var vals = [];
			console.log(buffer.length,sections,len);
...
window.addEventListener('load',function(){
	...
	send.onclick = function(){
		if( waveformArray != null && title.value != null && artist.value != null && genre.value != null ){
			send.style.backgroundColor = "gray";
			send.style.cursor = "initial";
			...
			xhr.open("POST","/api/context/test", false); xhr.send(formdata);
		}
	}
