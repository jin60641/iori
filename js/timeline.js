...
inits["timeline"] = {
	...
	openfile_paste : function(event,pid){
	},
	...
	openfile_reply : function(event,change){
		...
		reader.addEventListener("load",function(event){
			...
			if( change ){
				let output = $("#replyoutput_change_"+input.id.split('_').pop());
				...
				let output = $("#replyoutput_"+input.id.split('_').pop());
	...
	init : function(){
		...
		if( location.pathname.substr(0,6) == "/post/" ){
			if( Post.length ){
				Post = Post[0]
				postwrap.appendChild(this.makePost(Post));
			} else {
				location.href = "/";
			}
