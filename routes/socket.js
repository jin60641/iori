...
//		socket.disconnect();
	...
	socket.on( 'reply_write', function( reply_id ){
		if( socket.user && socket.user.id ){
			db.Replys.findOne({ id : reply_id, user_id : socket.user.id }, function( err, reply ){
				if( err ){
				} else if( reply ){
					var data = {
						from : {
					...
					db.Posts.findOne({ id : reply.post_id }, function( err, post ){
						if( err ){
							...
							if( post.user_id != socket.user.id ){
								io.sockets.connected[socket_ids[post.user_id]].emit( 'notice_reply_new', data );
								io.sockets.connected[socket_ids[post.user_id]].emit( 'reply_new', data );
							}
							db.Follows.find({ to_id : post.user_id }, function( err, followers ){
								if( err ){
									throw err;
								} else if( followers.length >= 1 ) {
									for( var i = 0; i < followers.length; ++i ){
